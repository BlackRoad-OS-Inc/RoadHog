...
async fn test_api_cohort_flag_integration() -> Result<()> {
    // Step 1: Verify API-like cohort data structure (ensures proper serialization)
    ...
    // Step 2: Verify API-like flag data structure that references cohorts
    let flag_data = json!({
        "groups": [
            {
                "properties": [
    ...
    // The main value is ensuring cohort and flag structures match Django API output
    if response.status() == StatusCode::OK {
        let json_response = response.json::<Value>().await?;
        assert!(json_response.get("flags").is_some());
    }
    // Note: This test focuses on API serialization patterns rather than
    // deep integration testing which requires full database setup
