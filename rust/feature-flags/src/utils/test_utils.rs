...
/// Result from creating a cohort via the Django serializer (`create_test_data.py`).
#[derive(Debug, Clone, Deserialize)]
pub struct DjangoCohortResult {
    #[serde(rename = "type")]
    #[allow(dead_code)]
...
/// Result from creating a feature flag via the Django serializer (`create_test_data.py`).
#[derive(Debug, Clone, Deserialize)]
pub struct DjangoFlagResult {
    #[serde(rename = "type")]
    #[allow(dead_code)]
    pub result_type: String,
    pub id: i32,
...
/// Parsed output from the `create_test_data.py` helper script.
#[derive(Debug, Deserialize)]
pub struct DjangoCreateResult {
...
impl TestContext {
    ...
    // ── Django serializer helpers ──────────────────────────────────
    // These call `create_test_data.py` which bootstraps Django and runs
    // cohort / flag creation through the real serializers, giving us
    // production-accurate `cohort_type`, bytecode, filter validation, etc.

    /// Run the `create_test_data.py` script with the given operations payload.
    /// Returns the parsed JSON output from the script.
    ///
    /// This is intentionally synchronous — it blocks while the Python subprocess
    /// runs, which is fine for test setup.
    pub fn run_django_create_script(
        &self,
        team_id: i32,
        operations: Vec<Value>,
    ) -> Result<DjangoCreateResult, Error> {
        let payload = json!({
            "team_id": team_id,
        ...
        // relative to the crate root (rust/feature-flags/).
        let script_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests")
            .join("create_test_data.py");
        ...
        let mut child = std::process::Command::new("python3")
            ...
            .spawn()
            .map_err(|e| anyhow::anyhow!("Failed to spawn create_test_data.py: {e}"))?;

        // Write the JSON payload to stdin, then drop to close it.
        {
            use std::io::Write;
            let stdin = child
                .stdin
                .as_mut()
                .ok_or_else(|| anyhow::anyhow!("Failed to open stdin for create_test_data.py"))?;
                ...
                Ok(None) => {
                    ...
                    if start.elapsed() > timeout {
                        ...
                        return Err(anyhow::anyhow!(
                            "create_test_data.py timed out after {}s. \
                             Check that Django can connect to the test database.",
                            timeout.as_secs()
                        ));
                    }
        ...
        if !output.status.success() {
            ...
            return Err(anyhow::anyhow!(
                "create_test_data.py failed (exit code {:?}):\nstderr: {}\nstdout: {}",
                output.status.code(),
                stderr,
                stdout,
            ));
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        let result: DjangoCreateResult = serde_json::from_str(&stdout).map_err(|e| {
            anyhow::anyhow!("Failed to parse create_test_data.py output: {e}\nRaw output: {stdout}")
        })?;

        if let Some(ref error) = result.error {
            return Err(anyhow::anyhow!("Django script reported error: {error}"));
        }

        Ok(result)
    }

    /// Create a cohort via the Django `CohortSerializer`, ensuring realistic
    /// `cohort_type`, bytecode, and filter validation — matching production behavior.
    pub fn create_cohort_via_django(
        &self,
        team_id: i32,
        name: &str,
        filters: Value,
        is_static: bool,
    ) -> Result<DjangoCohortResult, Error> {
        ...
        let result = self.run_django_create_script(team_id, operations)?;
        let cohort_value = result
            .results
            .into_iter()
            .next()
            .ok_or_else(|| anyhow::anyhow!("No results returned from Django script"))?;
        let cohort: DjangoCohortResult = serde_json::from_value(cohort_value)?;
        Ok(cohort)
    }

    /// Create a feature flag via the Django `FeatureFlagSerializer`, ensuring
    /// realistic filter validation — matching production behavior.
    pub fn create_flag_via_django(
        &self,
        team_id: i32,
        key: &str,
        name: &str,
        ...
        let flag_value = result
            ...
            .next()
            .ok_or_else(|| anyhow::anyhow!("No results returned from Django script"))?;
        let flag: DjangoFlagResult = serde_json::from_value(flag_value)?;
        Ok(flag)
    }

    /// Create multiple cohorts and flags in a single Django script invocation.
    /// More efficient than separate calls because it amortizes Django startup (~1-2s).
    ///
    /// Operations are processed in order. Use `"$cohort_N"` placeholders in flag
    /// filters to reference the Nth cohort created in the same batch (0-indexed).
    pub fn create_via_django_batch(
        &self,
        team_id: i32,
        operations: Vec<Value>,
    ) -> Result<DjangoCreateResult, Error> {
        self.run_django_create_script(team_id, operations)
    }
}
