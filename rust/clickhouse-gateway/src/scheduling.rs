...
impl Scheduler {
    ...
    /// the target ClickHouse host. For all other workloads, or on any EXPLAIN
    /// error, falls back to a heuristic based on SQL structure.
    pub async fn estimate_cost(
        ...
        if workload == "ONLINE" && self.online_explain_enabled {
            self.explain_cost(client, host, sql).await
        } else {
...
mod tests {
    ...
    fn test_parse_explain_response_no_index_returns_none() {
        ...
        let result =
            scheduler.parse_explain_response(&serde_json::to_string(&json).unwrap());
