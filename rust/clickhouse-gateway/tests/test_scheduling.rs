...
fn test_heuristic_simple_select() {
    let scheduler = make_scheduler();
    let cost = scheduler
        .heuristic_cost("SELECT count() FROM events WHERE team_id = 42 AND timestamp > now() - INTERVAL 1 DAY");
...
fn test_heuristic_distinct_adds_cost() {
    let scheduler = make_scheduler();
    let cost = scheduler
        .heuristic_cost("SELECT DISTINCT event FROM events WHERE team_id = 1");
