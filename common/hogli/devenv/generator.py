...
class MprocsGenerator(ConfigGenerator):
    ...
    def generate(self, resolved: ResolvedEnvironment, source_config: DevenvConfig | None = None) -> MprocsConfig:
        ...
        for name in self.registry.get_processes():
            ...
            # Special handling for dagster - point at generated workspace.yaml when locations are resolved
            if name == "dagster":
                proc_config = self._use_generated_workspace(proc_config, resolved)
...
printf '  {gray}Run {reset}{blue}hogli dev:setup{reset}{gray} to tailor this to your workflow.{reset}\\n'"""
    ...
    def _use_generated_workspace(self, proc_config: dict[str, Any], resolved: ResolvedEnvironment) -> dict[str, Any]:
        """Point the dagster process at the generated workspace.yaml when dagster_locations are resolved."""
        if not resolved.dagster_locations:
            return proc_config
        original_shell = proc_config.get("shell", "")
        if original_shell:
            proc_config = proc_config.copy()
            proc_config["shell"] = original_shell.replace(
                "$DAGSTER_HOME/workspace.yaml", ".posthog/.generated/workspace.yaml"
            )
        return proc_config
...
DAGSTER_LOCATION_MODULE_MAP: dict[str, str] = {
    "analytics_platform": "posthog.dags.locations.analytics_platform",
    "experiments": "posthog.dags.locations.experiments",
    "growth": "posthog.dags.locations.growth",
    "ingestion": "posthog.dags.locations.ingestion",
    "llm_analytics": "posthog.dags.locations.llm_analytics",
    "posthog_ai": "posthog.dags.locations.posthog_ai",
    "shared": "posthog.dags.locations.shared",
    "web_analytics": "posthog.dags.locations.web_analytics",
}


def get_generated_workspace_path() -> Path:
    """Get path for the generated Dagster workspace.yaml (sibling of generated mprocs.yaml)."""
    return get_generated_mprocs_path().parent / "workspace.yaml"


class DagsterWorkspaceGenerator(ConfigGenerator):
    """Generates Dagster workspace.yaml with only the locations needed for selected intents."""

    def generate(self, resolved: ResolvedEnvironment, source_config: DevenvConfig | None = None) -> dict[str, Any]:
        entries = [
            {"python_module": DAGSTER_LOCATION_MODULE_MAP[loc]}
            for loc in sorted(resolved.dagster_locations)
            if loc in DAGSTER_LOCATION_MODULE_MAP
        ]
        return {"load_from": entries}

    def save(self, config: dict[str, Any], output_path: Path) -> Path:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        header = "# Generated by hogli dev:generate — do not edit manually\n# Run 'hogli dev:setup' to reconfigure\n\n"
        with open(output_path, "w") as f:
            f.write(header)
