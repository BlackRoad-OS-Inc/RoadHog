...
class MprocsGenerator(ConfigGenerator):
    ...
    def __init__(self, registry: ProcessRegistry):
        ...
        self._dagster_workspace_yaml: str | None = None
    ...
    def generate(self, resolved: ResolvedEnvironment, source_config: DevenvConfig | None = None) -> MprocsConfig:
        ...
        for name in self.registry.get_processes():
            ...
            # Special handling for dagster - generate workspace.yaml with only resolved locations
            if name == "dagster":
                proc_config = self._configure_dagster_workspace(proc_config, resolved)
...
printf '  {gray}Run {reset}{blue}hogli dev:setup{reset}{gray} to tailor this to your workflow.{reset}\\n'"""
    ...
    def _configure_dagster_workspace(
        self, proc_config: dict[str, Any], resolved: ResolvedEnvironment
    ) -> dict[str, Any]:
        """Generate a tailored dagster workspace.yaml based on resolved dagster locations.

        Writes a workspace.yaml to .posthog/.generated/dagster-workspace.yaml and
        updates the dagster shell command to use it via DAGSTER_WORKSPACE env var.
        """
        if not resolved.dagster_locations:
            return proc_config

        locations = sorted(resolved.dagster_locations)

        # Build workspace.yaml content
        workspace_data = {
            "load_from": [{"python_module": loc} for loc in locations],
        }
        workspace_yaml = yaml.dump(workspace_data, default_flow_style=False, sort_keys=False)

        # Write workspace file next to the generated mprocs.yaml
        workspace_path = Path(".posthog") / ".generated" / "dagster-workspace.yaml"

        # Store for writing during save()
        self._dagster_workspace_yaml = workspace_yaml

        # Update shell to point dagster at the generated workspace
        original_shell = proc_config.get("shell", "")
        if original_shell and "$DAGSTER_HOME/workspace.yaml" in original_shell:
            proc_config["shell"] = original_shell.replace("$DAGSTER_HOME/workspace.yaml", str(workspace_path))
    ...
    def save(self, config: MprocsConfig, output_path: Path) -> Path:
        ...
        # Write dagster workspace file if generated
        if self._dagster_workspace_yaml:
            workspace_path = output_path.parent / "dagster-workspace.yaml"
            with open(workspace_path, "w") as f:
                f.write("# Generated by hogli dev:generate — do not edit\n")
                f.write("# To change which dagster locations load, run: hogli dev:setup\n\n")
                f.write(self._dagster_workspace_yaml)
