...
def _defer_pak_invalidation(secure_value: str) -> None:
    """Invalidate a PAK's auth cache entry synchronously after commit."""
    from posthog.tasks.team_access_cache_tasks import invalidate_token_sync
...
def personal_api_key_saved(sender, instance: "PersonalAPIKey", created, **kwargs):
    _handle_api_key_saved(instance, created, _PAK_CACHE_RELEVANT_FIELDS, _defer_pak_invalidation, **kwargs)
...
def personal_api_key_deleted(sender, instance: "PersonalAPIKey", **kwargs):
    if instance.secure_value:
        _defer_pak_invalidation(instance.secure_value)


# --- ProjectSecretAPIKey signal handlers ---


def _defer_psak_invalidation(secure_value: str) -> None:
    """Invalidate a PSAK's auth cache entry synchronously after commit."""
    from posthog.tasks.team_access_cache_tasks import invalidate_token_sync
...
def project_secret_api_key_saved(sender, instance: "ProjectSecretAPIKey", created, **kwargs):
    _handle_api_key_saved(instance, created, _PSAK_CACHE_RELEVANT_FIELDS, _defer_psak_invalidation, **kwargs)
...
def project_secret_api_key_deleted(sender, instance: "ProjectSecretAPIKey", **kwargs):
    if instance.secure_value:
        _defer_psak_invalidation(instance.secure_value)
