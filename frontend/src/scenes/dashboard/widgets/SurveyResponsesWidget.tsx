        ...
        <div className="p-4 space-y-3 h-full overflow-auto">
            ...
            {questions.length > 0 && (
                <div className="space-y-2">
                    <div className="text-xs font-medium text-muted uppercase">Questions ({questions.length})</div>
                    {questions.map((question, index) => (
                        <div key={index} className="p-2 rounded border bg-surface-secondary text-sm">
                            <div className="font-medium truncate">{question.question}</div>
                            <div className="text-xs text-muted mt-0.5">{questionTypeLabel(question.type)}</div>
                        </div>
                    ))}
                </div>
            )}
