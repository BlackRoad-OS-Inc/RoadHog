...
func TestSearch_liveUpdate(t *testing.T) {
	m := readyModel(t, "backend")
	p, _ := m.mgr.Get("backend")
	...
	// New matching line arrives — recomputeSearch should find it
	p.AppendLine("another error")
	m = update(m, process.OutputMsg{Name: "backend", Line: "another error"})
	if len(m.searchMatches) != 2 {
